        ...
        <div className="flex">
          {
            task?.team.map((m, index) => (
                <div className={clsx("w-7 h-7 rounded-full text-white flex items-center justify-center text-sm -mr-1", BGS[index % BGS.length])} key={index}>
                  <UserInfo user={m}/>
                </div>
            ))
          }
...
const Dashboard = () => {
  ...
  const Card = ({icon, bg, label, count}) => {
    return( 
      <div className="w-full h-32 bg-white p-5 shadow-md rounded-md flex items-center">
